...
export const deleteAllSessionsAPI = async (
  ...
  try {
    ...
    const sessionsResponse = await getAllSessionsAPI(base, type, componentId, dbId, authToken)
    const sessions = sessionsResponse.data || []
    
    if (sessions.length === 0) {
      return { success: true, deletedCount: 0, message: 'No sessions to delete' }
    }

    // Delete all sessions in parallel
    const deletePromises = sessions.map(session =>
      deleteSessionAPI(base, dbId, session.session_id, authToken)
    )
    ...
    return {
      ...
      message: `Deleted ${successful} of ${sessions.length} sessions`
    }
  } catch (error) {
    throw new Error(`Failed to delete all sessions: ${error instanceof Error ? error.message : String(error)}`)
  }
}
