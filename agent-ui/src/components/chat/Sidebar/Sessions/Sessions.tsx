...
import { deleteAllSessionsAPI } from '@/api/os'
...
const Sessions = () => {
  ...
  const handleDeleteAllSessions = async () => {
    if (!(selectedEndpoint && (agentId || teamId || dbId))) return
    
    setIsDeletingAll(true)
    try {
      const componentId = agentId || teamId || ''
      const entityType = agentId ? 'agent' : 'team'
      ...
      if (result.success) {
        ...
        toast.success(`Successfully deleted ${result.deletedCount} sessions`)
      } else {
        toast.error(`Failed to delete some sessions: ${result.message}`)
      }
    } catch (error) {
      toast.error(
        `Failed to delete all sessions: ${error instanceof Error ? error.message : String(error)}`
      )
    } finally {
